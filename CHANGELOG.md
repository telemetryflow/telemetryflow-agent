...
[![Coverage](https://img.shields.io/badge/Coverage-88.4%25-green.svg)](CHANGELOG.md)
...
- **OTLP metric export pipeline (critical)**: Collectors were calling `Collect()` and discarding the results — metrics never reached the OTLP exporter or Prometheus bridge. Added `MetricForwarder` (`internal/exporter/metric_forwarder.go`) that periodically collects from all running collectors and forwards `[]collector.Metric` to both the OTLP HTTP bridge and Prometheus bridge. The forwarder is auto-wired in `agent.New()` when collectors are present, uses the system collector's interval, and starts/stops with the agent lifecycle. Both `OTLPMetricBridge` and `MetricsBridge` are now live instead of dead code.
- **DB collector metric data loss (critical)**: All 7 database collectors (PostgreSQL, MySQL, MongoDB, MSSQL, CockroachDB, TimescaleDB, SQLite3) had internal ticker loops that called `collectAll*()` helpers (query analytics, schema/table stats, index stats, chunk/job metrics, integrity checks) and **discarded the results**. These metric families were collected but never exported. Fixed by merging `collectAll*()` calls into `Collect()` so the forwarder exports them. Removed redundant internal tickers from `Start()` — the forwarder now drives all collection.
- **Infrastructure collector double-collection**: System, node_exporter, cAdvisor, Docker, eBPF, and Kubernetes collectors had internal `Start()` ticker loops that called `Collect()` and discarded results, causing double API/HTTP calls per cycle and corrupted rate/delta calculations (e.g. `prevCounters`/`lastStats` consumed twice). Removed internal tickers — `Start()` now only manages lifecycle (subprocess, BPF programs, connections). The forwarder is the sole collection driver.
- **Aurora duplicate metric export**: Aurora collector had both a push path (internal buffer + flush ticker to custom endpoint) and a poll path (`Collect()` → forwarder). CloudWatch metrics were exported twice. Removed internal metrics/PI/push tickers. PI metrics are now folded into `Collect()`. Topology sync ticker retained.
- **RDS PostgreSQL duplicate metric export**: Same push/poll duplication as Aurora. Activity, query analytics, and table stats were pushed via `reporter.Submit()` AND returned by `Collect()` for the forwarder. Removed internal tickers, folded query analytics and table stats into `Collect()`.
...
- **OTLP aggregation type mismatch (critical)**: `buildAggregation()` returned pointer types (`*metricdata.Gauge[float64]`, `*metricdata.Sum[float64]`) but the OTel SDK v1.43.0 OTLP transform pipeline type-switches on **value** types (`metricdata.Gauge[float64]`, `metricdata.Sum[float64]`). Go type switches do not match `*T` against `T`, so every single metric — system, DB, log — failed with `unknown aggregation: *metricdata.Gauge[float64]` / `*metricdata.Sum[float64]` and was dropped. Fixed by returning value types instead of pointers. Also corrected counter temporality from `DeltaTemporality` to `CumulativeTemporality` — collectors report cumulative totals (e.g. total bytes sent since process start), not per-interval deltas.
- **Metric forwarder visibility (critical)**: Successful OTLP exports logged at `Debug` level, making it impossible to verify metrics were flowing at production log levels. Changed to `Info` level with cumulative totals (`total_exports`, `total_metrics`). Also added `Debug`-level log when all collectors return empty metrics (with running collector count) for diagnostics.
- **Prometheus bridge label cardinality panic (critical)**: `MetricsBridge.UpdateMetrics()` derived label names from whichever metric arrived first for a given metric name. When a subsequent metric of the same name arrived with different label cardinality (e.g. one Kubernetes pod reports a `node` label and another does not), `GaugeVec.With()` panicked with `inconsistent label cardinality: expected N label values but got M`, crashing the agent. Fixed by: (1) tracking canonical (sorted) label names per metric name, (2) normalizing all incoming labels to match the canonical set before calling `.With()` — missing labels are padded with `""`, extra labels are dropped. The first metric's label set becomes canonical for each metric name.
...
- **Dockerfile OS hardening**: `dist-upgrade` applies all available Debian trixie-security patches at build time. Vulnerable Perl modules (Archive::Tar, IO::Compress, IO::Uncompress, HTTP::Tiny) removed from image — no runtime impact. Added `.trivyignore` to suppress accepted findings (perl-base package version vs actual files, curl connection-reuse CVEs not applicable to agent's usage pattern)
- **CVE-2025-4270 (MEDIUM)**: HTTP::Tiny < 0.093 CRLF in HTTP response headers — module stripped from Docker image (GHSA alert #226)
- **CVE-2024-37370 (MEDIUM)**: krb5 integer underflow in `berval2tl_data()` leads to heap out-of-bounds read — krb5 is transitive dependency (libsasl2/libcurl), agent does not use Kerberos auth; added to `.trivyignore` pending Debian trixie patch (GHSA alerts #222-225)
- **CVE-2024-37371 (MEDIUM)**: krb5 integer underflow variant — same as above, transitive dependency not directly used
...
- **Collector lifecycle architecture**: All 15 collectors (system, nodeexporter, cadvisor, docker, ebpf, kubernetes, postgresql, mysql, mongodb, mssql, cockroachdb, timescaledb, sqlite3, aurora, rds_postgresql) refactored. `Start()` now only manages lifecycle (init connections, load BPF programs, topology sync). `Collect()` is the sole data-path method, polled by the `MetricForwarder`. This eliminates dual collection loops, rate corruption from double `prevCounters` reads, and silent metric data loss.

...
func buildConnString(cfg config.CockroachDBInstanceConfig) string {
	password := resolveEnvVars(cfg.Password)
	dsn := fmt.Sprintf("postgres://%s:%s@%s:%d/%s?sslmode=%s",
		cfg.User, password, cfg.Host, cfg.SQLPort, cfg.Database, cfg.SSLMode,
	)
	if cfg.SSLRootCert != "" {
...
func (c *CockroachDBCollector) ensureConnection(ctx context.Context, inst *crdbInstance) (*pgxpool.Pool, error) {
	if inst.pool != nil {
	...
	if !inst.lastConnErr.IsZero() {
		...
		if time.Since(inst.lastConnErr) < wait {
			return nil, fmt.Errorf("cockroachdb %s: in back-off (retry in %s)",
	...
	if err != nil {
		...
		return nil, fmt.Errorf("cockroachdb %s: parse config: %w", inst.config.Name, err)
	}
	...
	if err != nil {
		c.advanceBackoff(inst)
		return nil, fmt.Errorf("cockroachdb %s: create pool: %w", inst.config.Name, err)
	}

	if err := pool.Ping(ctx2); err != nil {
		pool.Close()
		c.advanceBackoff(inst)
		return nil, fmt.Errorf("cockroachdb %s: ping: %w", inst.config.Name, err)
	}

	inst.pool = pool
	inst.backoff = 0
	inst.lastConnErr = time.Time{}
	c.logger.Info("Connected to CockroachDB instance",
		zap.String("instance", inst.config.Name),
		zap.String("host", inst.config.Host),
		zap.Int("port", inst.config.SQLPort),
	)
	return pool, nil
}

func (c *CockroachDBCollector) closeConnection(inst *crdbInstance) {
	if inst.pool != nil {
		inst.pool.Close()
		inst.pool = nil
	}
...
func (c *CockroachDBCollector) advanceBackoff(inst *crdbInstance) {

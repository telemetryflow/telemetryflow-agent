...
func instanceLabels(inst *tsdbInstance) map[string]string {
	labels := map[string]string{
		"timescaledb_instance": inst.config.Name,
		"timescaledb_host":     inst.config.Host,
	}
	if inst.pgVersionS != "" {
		labels["postgresql_version"] = inst.pgVersionS
	}
	if inst.tsdbVer != "" {
		labels["timescaledb_version"] = inst.tsdbVer
	}

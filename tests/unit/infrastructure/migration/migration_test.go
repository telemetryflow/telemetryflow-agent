...
func markerMigration(from, to, name string) migration.Migration {
...
func TestRegistry_RegisterValidatesRequiredFields(t *testing.T) {
	...
	err := r.Register(migration.Migration{Name: "x", FromVersion: "1.2.0", ToVersion: "1.3.0"})
	require.Error(t, err)

	err = r.Register(migration.Migration{Name: "x", FromVersion: "", ToVersion: "1.3.0", Apply: noop})
	require.Error(t, err)

	err = r.Register(migration.Migration{Name: "", FromVersion: "1.2.0", ToVersion: "1.3.0", Apply: noop})
	require.Error(t, err)
}
...
func TestRegistry_ApplyAllPropagatesApplyError(t *testing.T) {
	r := migration.NewRegistry()
	require.NoError(t, r.Register(migration.Migration{
		FromVersion: "1.2.0",
		ToVersion:   "1.3.0",
		Name:        "boom",
...
func TestRegistry_ListReturnsSortedCopy(t *testing.T) {
	...
	list[0] = migration.Migration{Name: "mutated"}
	again := r.List()
...
func TestDetectVersion_ReadsAgentVersionFromYAML(t *testing.T) {
	in := []byte("agent:\n  version: \"1.3.0\"\n  id: foo\n")
	assert.Equal(t, "1.3.0", migration.DetectVersion(in))
}

func TestDetectVersion_DefaultsToLegacyWhenAbsent(t *testing.T) {
	assert.Equal(t, migration.LegacyVersion, migration.DetectVersion([]byte("agent:\n  id: foo\n")))
	assert.Equal(t, migration.LegacyVersion, migration.DetectVersion([]byte("")))
	assert.Equal(t, migration.LegacyVersion, migration.DetectVersion(nil))
}
...
func TestCompareVersion(t *testing.T) {
	...
	for _, c := range cases {
		...
		t.Run(c.a+"_vs_"+c.b, func(t *testing.T) {
			got := migration.CompareVersion(c.a, c.b)

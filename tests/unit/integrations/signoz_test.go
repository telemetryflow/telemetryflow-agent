// Package integrations_test provides unit tests for TelemetryFlow Agent SigNoz integration.
...
func TestNewSigNozExporter(t *testing.T) {
	logger := zap.NewNop()
	...
	assert.Equal(t, "signoz", exporter.Name())
	assert.Equal(t, "observability", exporter.Type())
	assert.True(t, exporter.IsEnabled())
}

func TestSigNozExporterInit(t *testing.T) {
	logger := zap.NewNop()
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			exporter := integrations.NewSigNozExporter(tt.config, logger)
...
func TestSigNozExporterHealth(t *testing.T) {
	logger := zap.NewNop()
	ctx := context.Background()

	config := integrations.SigNozConfig{Enabled: false}
	exporter := integrations.NewSigNozExporter(config, logger)
...
func TestSigNozExporterClose(t *testing.T) {
	logger := zap.NewNop()
	ctx := context.Background()
...
func TestSigNozExporterExportMetrics(t *testing.T) {
	logger := zap.NewNop()
	ctx := context.Background()
...
func TestSigNozExporterExportTraces(t *testing.T) {
	logger := zap.NewNop()
	ctx := context.Background()
...
func TestSigNozExporterExportLogs(t *testing.T) {
	logger := zap.NewNop()
	ctx := context.Background()
...
func TestSigNozExporterExport(t *testing.T) {
	logger := zap.NewNop()
	ctx := context.Background()
...
func TestSigNozNotInitializedOrEnabled(t *testing.T) {
	logger := zap.NewNop()
	ctx := context.Background()
...
func TestSigNozConfigDefaults(t *testing.T) {
	config := integrations.SigNozConfig{}
	assert.False(t, config.Enabled)
	assert.Empty(t, config.AccessToken)
}

func BenchmarkNewSigNozExporter(b *testing.B) {
	logger := zap.NewNop()
	config := integrations.SigNozConfig{
		Enabled:  true,
		Endpoint: "http://localhost:4318",
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		integrations.NewSigNozExporter(config, logger)
	}
}

// External black-box unit tests for the Elasticsearch collector.
//
...
import (
	...
	"github.com/telemetryflow/telemetryflow-agent/internal/collector/elasticsearch"
...
// newESCollector constructs a collector with sane defaults and ensures it is
// stopped during test cleanup.
func newESCollector(t *testing.T, instances []config.ElasticsearchInstance) *elasticsearch.ElasticsearchCollector {
	t.Helper()
	c := elasticsearch.NewElasticsearchCollector(config.ElasticsearchCollectorConfig{
		Enabled:   true,
...
func TestCollectorLifecycle(t *testing.T) {
	c := elasticsearch.NewElasticsearchCollector(config.ElasticsearchCollectorConfig{}, zap.NewNop())
	const wantName = "elasticsearch"
	if c.Name() != wantName {
...
func TestCollectorSatisfiesInterface(t *testing.T) {
	var _ collector.Collector = (*elasticsearch.ElasticsearchCollector)(nil)
}

func TestCollectNoInstances(t *testing.T) {
	c := newESCollector(t, nil)
...
func TestClusterHealthMetricsEmitted(t *testing.T) {
	...
	c := newESCollector(t, []config.ElasticsearchInstance{{Name: "es1", URL: srv.URL}})
	ms, err := c.Collect(context.Background())
	if err != nil {
		t.Fatalf("collect: %v", err)
	}
	...
	if state := mustGet(t, ms, "db.elasticsearch.state").Value; state != 1 {
		t.Fatalf("state = %v, want 1", state)
	}
	// Cluster health metrics. green=2.
	cases := map[string]float64{
		"db.elasticsearch.cluster_status":        2,
		"db.elasticsearch.nodes_total":           3,
		"db.elasticsearch.nodes_data":            2,
		"db.elasticsearch.shards_active":         20,
		"db.elasticsearch.shards_active_primary": 10,
		"db.elasticsearch.shards_relocating":     0,
		"db.elasticsearch.shards_initializing":   1,
		"db.elasticsearch.shards_unassigned":     2,
		"db.elasticsearch.pending_tasks":         5,
	}
	...
	// es_cluster label is promoted from cluster_name in the health response.
	m := mustGet(t, ms, "db.elasticsearch.nodes_total")
	if m.Labels["es_cluster"] != "test-cluster" {
		t.Fatalf("es_cluster label = %q, want %q", m.Labels["es_cluster"], "test-cluster")
	}
	if m.Labels["db_system"] != "elasticsearch" {
		t.Fatalf("db_system label = %q, want elasticsearch", m.Labels["db_system"])
	}
}
...
func TestYellowClusterStatus(t *testing.T) {
	...
	// yellow = 1
	if v := mustGet(t, ms, "db.elasticsearch.cluster_status").Value; v != 1 {
...
func TestNodeStatsMetricsEmitted(t *testing.T) {
	...
	nodeMetrics := []string{
		"db.elasticsearch.node.heap_used_percent",
		"db.elasticsearch.node.docs_count",
		"db.elasticsearch.node.store_size_bytes",
		"db.elasticsearch.node.search_query_total",
		"db.elasticsearch.node.indexing_total",
	}
	for _, name := range nodeMetrics {
		if got := mustGet(t, ms, name, "es_node", "node-1").Value; got < 0 {
			t.Fatalf("%s for node-1 missing or negative: %v", name, got)
		}
	}

	// Spot-check a couple of values + types.
	if v := mustGet(t, ms, "db.elasticsearch.node.heap_used_percent", "es_node", "node-1").Value; v != 65 {
		t.Fatalf("node-1 heap_used_percent = %v, want 65", v)
	}
	if v := mustGet(t, ms, "db.elasticsearch.node.docs_count", "es_node", "node-1").Value; v != 10000 {
		t.Fatalf("node-1 docs_count = %v, want 10000", v)
	}
	if v := mustGet(t, ms, "db.elasticsearch.node.store_size_bytes", "es_node", "node-2").Value; v != 524288 {
		t.Fatalf("node-2 store_size_bytes = %v, want 524288", v)
	}

	// Counters for cumulative totals.
	if m := mustGet(t, ms, "db.elasticsearch.node.search_query_total", "es_node", "node-1"); m.Type != collector.MetricTypeCounter {
		t.Fatalf("search_query_total type = %q, want counter", m.Type)
	}
	if m := mustGet(t, ms, "db.elasticsearch.node.indexing_total", "es_node", "node-1"); m.Type != collector.MetricTypeCounter {
	...
	for _, node := range []string{"node-1", "node-2"} {
		for _, name := range nodeMetrics {
			if _, ok := findMetric(ms, name, "es_node", node); !ok {
...
func TestUnauthorizedReturnsStateZero(t *testing.T) {
	...
	if state := mustGet(t, ms, "db.elasticsearch.state").Value; state != 0 {
...
func TestNodesStatsFailureKeepsClusterMetrics(t *testing.T) {
	...
	if v := mustGet(t, ms, "db.elasticsearch.nodes_total").Value; v != 3 {
...
func TestClusterHealthOnlySkipsNodeStats(t *testing.T) {
	srv := newHappyServer(t)
	c := newESCollector(t, []config.ElasticsearchInstance{{
		Name:              "es1",
		URL:               srv.URL,
...
func TestBasicAuth(t *testing.T) {
	const user, pass = "elastic", "s3cr3t"
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		u, p, ok := r.BasicAuth()
		if !ok || u != user || p != pass {
			w.WriteHeader(http.StatusUnauthorized)
	...
	bad := newESCollector(t, []config.ElasticsearchInstance{{
		Name: "es1", URL: srv.URL, Username: "wrong", Password: "nope",
	}})
	...
	good := newESCollector(t, []config.ElasticsearchInstance{{
		Name: "es1", URL: srv.URL, Username: user, Password: pass,
	}})
...
func TestDefaultTimeoutApplied(t *testing.T) {
	...
	srv := newHappyServer(t)
	c := newESCollector(t, []config.ElasticsearchInstance{{
		Name: "es1", URL: srv.URL, Timeout: 0,
	}})
	ms, err := c.Collect(context.Background())
	if err != nil {
		t.Fatalf("collect: %v", err)
